...
;
for i_time=200,300,10 do begin
;
if (hard eq 0) then window,i_time/10-20 else begin
  !p.charsize=1.5
  !p.charthick=3
  !x.thick=3
  !y.thick=3
  ...
  !p.thick=3
  set_plot,'ps'
  device,filename=folder_name+'width_and_kurtosis_time_'+string(format='(i3)',i_time)+'.eps'
endelse
;
...
;
plot,v_excess(i_time,*),red_kurt(i_time,*),xtitle='Excess Width (in units of v!dth!n)',ytitle='Reduced Kurtosis'
xyouts,4.5,-0.5,'Time = '+string(format='(i3)',i_time)
xyouts,4.5,-1,'Slit positions as indicated'
for i_pos=0,n_pos-1,3 do xyouts,v_excess(i_time,i_pos),red_kurt(i_time,i_pos),string(format='(i2)',i_pos)
;
