...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;                                              
saturationlimit = 4000 ; figure out how to properly do this for different datasets
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;
;
spectrumnum = 1 ; 0 for SiIV1394, 1 for O_IV1401, 2 for SiIV1403, (3 for O_I_1355?)
regionnum = 1 ; 0 for Region 1, 1 for Region 2, etc.
;
SiIV1394r = ['SiIV1394_25Oct2014_Region1.fits', 'SiIV1394_25Oct2014_Region2.fits']
O_IV1401r = ['O_IV1401_25Oct2014_Region1.fits', 'O_IV1401_25Oct2014_Region2.fits']
SiIV1403r = ['SiIV1403_25Oct2014_Region1.fits', 'SiIV1403_25Oct2014_Region2.fits']
fitsfiles = [[SiIV1394r], [O_IV1401r], [SiIV1403r]] ; combines each spectra into a 2D array
file_name = fitsfiles[spectrumnum][regionnum] ; selects the ith spectra in the jth region, i = spectrumnum, j = regionnum
;
...
;
inst_width = 0.01214 * sqrt(2.) ; instrumental line width (1 standard deviation) * 1/e half width (SQRT2!!!) in Angstroms
;
...
warning_flag=intarr(n_time,n_elements(intensity(0,0,*))) ; set to 1 if count in any channel exceeds saturationlimit
fluence_pixel=dblarr(n_pixel) ; event-integrated fluence for each pixel, fluence = flux * time; multiply by dt???
;
...
for i_time=0,n_time-1 do begin
  for slit_pos=0,n_pixel-1 do begin
    for i_lambda=0,n_lambda-1 do begin
      if intensity(i_lambda,i_time,slit_pos) gt saturationlimit then warning_flag(i_time,slit_pos)=1
    endfor
  endfor
endfor
;
...
for i_time=0,n_time-1 do begin
  for slit_pos=0,n_pixel-1 do begin
    if warning_flag(i_time,slit_pos) eq 0 then $
      intensity_pixel_plot(i_time,slit_pos)=intensity_pixel(i_time,slit_pos) else $ ; (surface plot of each array shows intensity of each pixel as function of pos & t)
      intensity_pixel_plot(i_time,slit_pos)=-999.
  endfor
endfor
;
; determine time-integrated fluence for each pixel in the image during intensity peak times (selects the most energetic pixel at that time)
;
for slit_pos=0,n_pixel-1 do fluence_pixel(slit_pos)=total(intensity_pixel(*,slit_pos))
;
; determine lightcurve for the whole flare (including saturated "saturationlimit" values)
;
...
;
; find time of maximum overall intensity and brightest three pixels (with no warning flag) *at that time* (can choose any number or function of bright pixels)
;
...
for i_slit_pos=0,2 do begin
  ...
  ;
  ; width of principal component must be positive and less than 0.2 Angstroms (vary from event to event?)
  ;
